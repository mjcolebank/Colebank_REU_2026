...
# ──────────────────────────────────────────────────────────────────────
# 4. STANDARD CNN ARCHITECTURE (NO BAYESIAN)
# ──────────────────────────────────────────────────────────────────────
...
class HeartDiseaseModel(nn.Module):
    def __init__(self, sequence_length=2500, num_classes=2, in_channels=12, demog_dim=8, demo_embed_dim=16):
        ...
        # Demographic Encoder matching demographic inputs dynamically
        self.demog_features = nn.Sequential(
            nn.Linear(demog_dim, 32),
            nn.BatchNorm1d(32),
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(32, demo_embed_dim),
            nn.BatchNorm1d(demo_embed_dim),
            nn.ReLU()
        )

        # Replicated Convolution Blocks
        self.conv1 = nn.Conv1d(in_channels, 16, kernel_size=7, padding=3)
        self.bn1 = nn.BatchNorm1d(16)
        self.film1 = FiLMLayer(demo_embed_dim, 16)
        self.act1 = nn.ReLU()
        self.drop1 = nn.Dropout(0.5)
        self.pool1 = nn.AdaptiveAvgPool1d(250)

        self.conv2 = nn.Conv1d(16, 32, kernel_size=5, padding=2)
        self.bn2 = nn.BatchNorm1d(32)
        self.film2 = FiLMLayer(demo_embed_dim, 32)
        self.act2 = nn.ReLU()
        self.drop2 = nn.Dropout(0.6)
        self.pool2 = nn.AdaptiveAvgPool1d(25)

        self.conv3 = nn.Conv1d(32, 32, kernel_size=3, padding=1)
        self.bn3 = nn.BatchNorm1d(32)
        self.film3 = FiLMLayer(demo_embed_dim, 32)
        self.act3 = nn.ReLU()
    ...
    def forward(self, ts_input, demog_input):
        demo_emb = self.demog_features(demog_input)

        # Block 1
        x = self.conv1(ts_input)
        x = self.bn1(x)
        x = self.film1(x, demo_emb)
        x = self.act1(x)
        x = self.drop1(x)
        x = self.pool1(x)
...
def train_classifier(model, train_loader, val_loader, epochs=60, learning_rate=1e-4):
    ...
    # FIXED: CosineAnnealingWarmRestarts steps per epoch natively without parameters
    ...
    for epoch in range(epochs):
        ...
        # FIXED: Removed dynamic loss validation argument to resolve native TypeError
        scheduler.step()
...
def generate_and_display_comparative_metrics(model, train_loader, test_loader):
    ...
    plt.tight_layout()
    plt.savefig("earlyfusion_cnn_standardized_metrics.png", dpi = 150)
...
if __name__ == "__main__":
    ...
    # FIXED: Replaced unsafe .iloc lookup with index-accurate .loc
    ...
    # FIXED: Instantiated correct model class name, passing valid demographic layer inputs
    model = HeartDiseaseModel(
        sequence_length=X_ts_train.shape[2],
        in_channels=X_ts_train.shape[1],
        demog_dim=X_demo_train.shape[1],
        num_classes=2,
        demo_embed_dim=16 #32 MJC CHANGE
    )
